...
This fork fixes an issue when association joins are used in the scope preceding
a call to all_tag_counts. This will be the case when searchlogic is used to
limit the scope of the tags using a conditions such as:

	Bar.foo_name_equals('foo1').all_tag_counts
